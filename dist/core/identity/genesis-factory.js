...
export class GenesisFactory {
     ...
     */
    static async spawnAgentIdentity(agentName, agentType) {
     ...
     */
    static async mintAgent(agentName, dna) {
        return this.spawnAgentIdentity(agentName, dna.role || "BountyHunter");
    }
    static distributeThresholdShares(privateKey) {
    ...
    static getDefaultPermissions(type) {
        const base = ["read:blackboard", "sign:tasks"];
        if (type === "BountyHunter")
            return [...base, "exec:arbitrage", "pay:pi"];
        if (type === "SaaSFactory")
            return [...base, "deploy:vercel", "write:code"];
        return base;
    }
}
