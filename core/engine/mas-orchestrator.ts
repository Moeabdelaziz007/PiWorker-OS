...
import { PlanStep } from './prompt-compiler';
...
export class MASOrchestrator extends EventEmitter {
  ...
  private ai: SovereignAIOrchestrator;
  ...
  constructor() {
    ...
    this.ai = SovereignAIOrchestrator.getInstance();
  ...
  public async orchestrateTask<T>(
    ...
    try {
      ...
      // 0. مرحلة التفكير الاستراتيجي (Sovereign AI Router)
      console.log(`[Orchestrator] ${ceo.name} is reasoning about the task via Sovereign AI Router...`);
      const strategy = await this.ai.dispatch(
        `Develop a sovereign strategy for task: ${JSON.stringify(taskData)} using skill: ${skill.name}`,
        {
          providerPreference: ceo.role === 'ceo' ? [AIModelProvider.GEMINI, AIModelProvider.DEEPSEEK] : [AIModelProvider.GROQ],
          maxTokens: 2000,
          temperature: 0.7
        }
      );
      console.log(`[Strategy] ${strategy.content}`);
      ...
      TelemetryLogger.log('INFO', 'TASK_EXECUTION', {
        ...
        prediction: simResult.estimatedRevenueUsd,
        actual: actualRoi,
  ...
  public async orchestrateGoal(
    goal: string,
    agents?: { ceo: Agent; executor: Agent; critic: Agent }
  ): Promise<PlanStep[]> {
    ...
    console.log(
      `[Orchestrator] Using Agents: CEO=${resolvedAgents.ceo.name}, Executor=${resolvedAgents.executor.name}`
    );
    ...
    try {
      ...
      if (pastExperiences.length > 0) {
        console.log(
          `[Orchestrator] 🧠 Recalled ${pastExperiences.length} relevant memories to optimize plan.`
        );
      }

      // 1. مرحلة التخطيط (Compilation)
      const contextEnhancedGoal =
  ...
  private async getOrBootstrapAgents(): Promise<{ ceo: Agent; executor: Agent; critic: Agent }> {
    ...
    const ceo =
      all.find((a) => a.role === 'ceo') ||
      this.createSyntheticAgent('CEO-Prime', 'ceo', 'StrategicPlanning');
    const executor =
      all.find((a) => a.role === 'executor') ||
      this.createSyntheticAgent('Executor-One', 'executor', 'BountyHunter');
    const critic =
      all.find((a) => a.role === 'critic') ||
      this.createSyntheticAgent('Critic-Alpha', 'critic', 'CodeAuditor');
  ...
  private async executeFinanceStep(step: PlanStep) {
    ...
    try {
      ...
      console.error(
        `[Finance] ❌ Fiscal error: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
      throw error;
    }
  }
